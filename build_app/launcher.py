...
# PUBLIC_DATA (project temp files) sits inside data/ ù one tree, easy backup.
...
# httpx parse NO_PROXY IPv6 tr?n ``::1`` thùnh port ``:1`` ? Whisper/HF crash.
...
# ?u tiùn GPU (CUDA/MPS); gi?i h?n thread CPU ph? ù trùnh ?? mùy
os.environ.setdefault("VIENEU_BACKEND", "auto")
os.environ.setdefault("OMP_NUM_THREADS", "2")
os.environ.setdefault("MKL_NUM_THREADS", "2")
os.environ.setdefault("OPENBLAS_NUM_THREADS", "2")
os.environ.setdefault("NUMEXPR_NUM_THREADS", "2")
# T?t async FFmpeg frame decoding ù trùnh "Assertion fctx->async_lock failed"
# (libavcodec/pthread_frame.c:173) khi VideoCapture m? video v?i multi-thread decoder.
# Khùng cù env var nùy: cv2 dùng thread_type=FRAME theo m?c ??nh ? assertion abort().
os.environ.setdefault("OPENCV_FFMPEG_CAPTURE_OPTIONS", "threads;1")
# Windows: KHùNG dùng MSMF vù MSMF t? ??ng chùn vi?n ?en (letterboxing) ho?c bùp mùo 
# khung hùnh video d?c lùm l?ch t?a ?? Bbox c?a OCR.
# S? d?ng FFmpeg v?i threads=1 ?ù kh?c ph?c ???c l?i pthread_frame.c.
if sys.platform == "win32":
...
def _crash_report(exit_code: int) -> str:
    ...
    lines: list[str] = [
        f"{APP_DISPLAY_NAME} ?ù thoùt b?t th??ng.",
        f"Mù: {exit_code} (0x{u:08X})",
        f"Log: {home / 'app.log'}",
        "",
    ]
    log = home / "app.log"
    try:
        if log.is_file():
            ...
            lines.append("(ch?a cù app.log)")
    except OSError as e:
        lines.append(f"(khùng ??c ???c app.log: {e})")
    return "\n".join(lines)


def show_copyable_crash(exit_code: int) -> None:
    """Popup l?i copy ???c ù khùng ?? APP t?t im l?ng sau native crash."""
    ...
    try:
        ...
        root = tk.Tk()
        root.title(f"{APP_DISPLAY_NAME} ù l?i (copy g?i ?? s?a)")
        root.geometry("720x480")
        root.attributes("-topmost", True)
        hint = tk.Label(
            root,
            text="APP ?ù thoùt b?t th??ng. Copy toùn b? n?i dung d??i g?i ?? s?a.",
            wraplength=680,
            justify="left",
        )
        ...
        def copy_all() -> None:
            ...
            btn.configure(text="?ù chùp")

        bar = tk.Frame(root)
        bar.pack(fill="x", padx=10, pady=(0, 10))
        btn = tk.Button(bar, text="Chùp l?i", command=copy_all)
        btn.pack(side="left")
        tk.Button(bar, text="?ùng", command=root.destroy).pack(side="right")
        root.mainloop()
        return
    except Exception:
        pass
    if sys.platform == "win32" and crash_file is not None:
        try:
            ...
            ctypes.windll.user32.MessageBoxW(
                0,
                f"APP ?ù thoùt b?t th??ng (0x{_unsigned_exit(exit_code):08X}).\n"
                f"Notepad ?ang m? log ?? copy:\n{crash_file}",
                f"{APP_DISPLAY_NAME} ù l?i",
                0x10 | 0x40000,
            )
            return
        except Exception:
...
# Runtime pack ???c kùch ho?t qua runtime/current.json; .venv-runtime ch? lù
# fallback t??ng thùch cho b?n c? trong lùc migration.
try:
...
# M? log ngay ù tr??c m?i import n?ng ?? crash lùc kh?i ??ng luùn ???c ghi.
if getattr(sys, "frozen", False):
    ...
    if _portable_fallback_from is not None:
        ...
        _runtime_log.write(
            f"[windows] Dùng d? li?u ng??i dùng ({_layout_reason}) / "
    ...
    # Luùn tee stderr vùo log k? c? khi stdout/stderr ?ù cù (child process)
    import io as _io
...
if runtime_site.is_dir():
    ...
    # Khùng nhùt nvidia/torch CUDA vùo PATH c?a ZM AI TOOL.exe (WebView2).
    # GPU ch?y trong worker .venv-runtime ù python.exe ?ù t? load CUDA DLL.
    if getattr(sys, "frozen", False):
        try:
...
if ocr_site.is_dir():
    # Khùng n?p CUDA/ORT vùo process c?a s?. GPU OCR/TTS/Whisper = worker .venv-runtime.
...
# Chocolatey ShimGen copy vùo _internal tr? `..\lib\ffmpeg\...` ? exit 4294967295.
# ??a th? m?c ffmpeg th?t (ngoùi bundle) lùn tr??c ?? bare `ffmpeg` khùng dùnh shim.
if sys.platform == "win32":
...
# Seed gi?ng zmAI ?i kùm; khùng ghi ?ù gi?ng ho?c metadata ng??i dùng ?ù s?a.
# ponytail: ki?m tra mtime ù khùng copy n?u target m?i h?n source (trùnh ch?m startup m?i l?n).
...
if bundled_voice_refs.is_dir():
    ...
    for source in bundled_voice_refs.iterdir():
        ...
        # ONEFILE gi?i nùn l?i m?i l?n ch?y ? mtime ngu?n LUùN m?i h?n, so mtime
        # s? ghi ?ù file ng??i dùng ?ù s?a. Ch? seed khi target ch?a cù, ho?c
        # n?i dung khùc vù target ch?a t?ng b? s?a (cùng size = b?n seed c?).
        ...
        try:
            if source.stat().st_size != target.stat().st_size:
                # B?n bundle ??i n?i dung: ch? ghi ?ù khi user ch?a s?a gù
                # (?ùnh d?u b?ng file .seeded c?nh bùn).
...
if sys.platform == "win32":
    try:
        ...
        # Resolve ffmpeg/ffprobe ? absolute path trong bundle ?? trùnh
        # trailing-space PATH trùn Windows ('ffprobe ' ? exit 4294967295).
        ...
        if _meipass:
            for _name in ("ffmpeg", "ffprobe", "uv"):
                ...
                if not os.path.isfile(_cand):
                    continue
                # B? Chocolatey shim (~400KB) ù copy vùo _internal thù gùy.
        ...
        class _PopenNoWindow(_OrigPopen):  # type: ignore[misc, valid-type]
            def __init__(self, *a, **kw):
                ...
                if args is not None and isinstance(args, (list, tuple)) and args:
                    ...
                    # N?u ch? lù bare name (khùng cù path separator), resolve t? bundle
...
def centered_xy(width: int, height: int) -> tuple[int, int]:
    """Gùc trùn-trùi ?? c?a s? n?m gi?a mùn hùnh chùnh."""
    sw, sh = 1920, 1080
    try:
        if sys.platform == "win32":
            import ctypes
...
def mark_update_ready(*_args: object) -> None:
    ...
    if not raw:
        return
    # ponytail: env var is set by launcher/CI only ù no untrusted input, no path guard needed.
    try:
...
def bind_desktop_events(window: object) -> None:
    """Forward native file drops with full paths to the React form."""
    mark_update_ready()
    try:
        ...
        def on_drop(event: dict) -> None:
            kind = siv_drop_kind(event.get("target") or {})
            if kind not in {"media", "audio", "timeline", "srt"}:
                # Event target attrs may be missing; React sets __zmSivDropKind on dragenter.
                try:
                    kind = str(window.evaluate_js("window.__zmSivDropKind||''") or "").strip()
                except Exception:
                    kind = ""
            if kind not in {"media", "audio", "timeline", "srt"}:
                return
            files = (event.get("dataTransfer") or {}).get("files") or []
            raw = take_dnd_path(files, _dnd_state.setdefault("paths", []))
            path = resolve_siv_drop_path(kind, raw)
            if not path:
                return
            payload = json.dumps({"kind": kind, "path": path}, ensure_ascii=False)
            window.run_js(
                "window.__zmSivDropKind=undefined;"
                f"window.dispatchEvent(new CustomEvent('zm-native-drop', {{detail: {payload}}}))"
            )

        window.dom.document.events.drop += DOMEventHandler(on_drop, True, True)
    except Exception:
        # Older pywebview builds still keep manual entry and native pickers working.
        traceback.print_exc()
...
def run_desktop() -> int:
    ...
    thread.start()
    _t0 = time.monotonic()
    print(f"{APP_DISPLAY_NAME} v{APP_VERSION} ù ch? API...", flush=True)
    try:
        if not wait_for_server(port):
            print(f"[desktop] API khùng kh?i ???c sau {time.monotonic()-_t0:.1f}s", flush=True)
            # Gi? c?a s? thùng bùo thay vù im l?ng exit
            try:
                webview.create_window(
                    f"{APP_DISPLAY_NAME} v{APP_VERSION}",
                    html=(
                        "<html><body style='font-family:sans-serif;padding:2rem'>"
                        f"<h2>Khùng m? ???c API / API failed to start</h2><p>{base}</p>"
                ...
                traceback.print_exc()
            return 1
        print(f"[desktop] API s?n sùng sau {time.monotonic()-_t0:.1f}s t?i {base}", flush=True)
        ...
        try:
            ...
            # pywebview c? cù th? khùng h? tr? icon= ho?c text_select=.
        ...
        try:
            ...
            traceback.print_exc()
        # webview.start() ch?n ??n khi user ?ùng c?a s? ù khùng thoùt vù l?i job n?n
        try:
            webview.start(bind_desktop_events, window, gui="edgechromium", debug=False)
        except Exception:
            try:
                webview.start(bind_desktop_events, window, debug=False)
            except Exception:
                traceback.print_exc()
                msg = (
                    "Khùng m? ???c c?a s?: Windows ?ang ch?n file t?i xu?ng (MOTW) "
                    "ho?c thi?u .NET/WebView2. Hùy gi?i nùn toùn b? ZIP vùo th? m?c cù quy?n ghi "
                    "vù ch?n Properties ? Unblock cho file ZIP. / Window startup failed: Windows "
...
if __name__ == "__main__":
    ...
    try:
        ...
        # M?i l?i kh?i ??ng: ghi log, khùng silent die
        traceback.print_exc()
        try:
