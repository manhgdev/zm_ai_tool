...
from portable_layout import migrate_windows_state, sync_windows_portable_root
...
class PortableLayoutTest(unittest.TestCase):
    def test_output_root_falls_back_when_saved_path_is_not_writable(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw:
            ...
            with patch.object(ui_preferences, "_PREFERENCES_PATH", data / "ui_preferences.json"), patch.dict(
                os.environ, {"VIDEO_CLONE_OUTPUT_ROOT": str(fallback)}
            ), patch.object(
                output_paths,
    ...
    def test_updater_swaps_only_immutable_payload_and_has_rollback(self) -> None:
        ...
        self.assertIn("VIDEO_CLONE_UPDATE_READY_FILE", script)
        self.assertIn("$env:VIDEO_CLONE_SUPERVISOR_CHILD = $null", script)
        ...
        for variable in ("VIDEO_CLONE_HOME", "VIDEO_CLONE_BUNDLE", "VIDEO_CLONE_VERSION"):
    ...
    def test_pre_portable_appdata_is_copied_into_new_layout(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            current = root / "portable" / "ZM_AIO_TOOL_v7.0.2-windows-x64"
            current.mkdir(parents=True)
            executable = current / "ZM AIO TOOL.exe"
            executable.touch()
            legacy = root / "local" / "VideoClone"
            (legacy / "public_data").mkdir(parents=True)
            (legacy / "public_data" / "project.json").touch()
            (legacy / ".python-runtime").mkdir()
            (legacy / ".python-runtime" / "python.exe").touch()

            used, _, errors = migrate_windows_state(
                current, executable, {"LOCALAPPDATA": str(root / "local")}
            )

            self.assertFalse(errors)
            self.assertEqual(used, [legacy])
            self.assertTrue((current / "data" / "public" / "project.json").is_file())
            self.assertTrue((current / "data" / ".python-runtime" / "python.exe").is_file())
            self.assertTrue((legacy / "public_data" / "project.json").is_file())
